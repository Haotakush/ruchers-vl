/* ============================================================
   ...
   Ce fichier gère :
   - Mise à jour des stats (compteurs)
   - Fréquence de visite par rucher (barres)
   - Recommandations / alertes
   - Timeline d'activité récente
   - Répartition par zone
   - Graphique des visites (6 derniers mois, canvas pur)
   ============================================================ */


/* ------------------------------------------------------------
   MISE À JOUR GLOBALE DU DASHBOARD
   ------------------------------------------------------------ */

/** Rafraîchir toutes les sections du dashboard */
...
/** Mettre à jour les 4 compteurs de stats */
...
/** Afficher les barres de fréquence de visite par rucher */
function updateFreqVisites() {
  const lastVisitMap = buildLastVisitMap();

  // Compter les visites par rucher
  const freqMap = {};
...
/** Afficher les alertes selon l'état des visites */
...
/** Afficher les 6 dernières activités (visites + traitements) */
...
/** Afficher les barres de répartition Bas / Mi-pentes / Hauts */
...
/* ------------------------------------------------------------
   GRAPHIQUE DES VISITES — Canvas pur (sans bibliothèque)
   ------------------------------------------------------------ */

/**
 * Dessiner le graphique en barres des visites sur 6 mois
 * Utilise l'API Canvas native, pas de dépendance externe
 */
function drawChartVisites() {
  ...
  // Générer les labels et données des 6 derniers mois
  const now    = new Date();
  const labels = [];
  const counts = [];
  ...
  counts.forEach((v, i) => {
    ...
    const r  = 4; // rayon arrondi haut des barres
    ...
    // Barres avec coins arrondis en haut uniquement
    ctx.beginPath();
    ctx.moveTo(x + r, y);
