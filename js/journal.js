...
function showICSPrompt(rappel, rucherLabel) {
  ...
  el.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:8px;align-items:flex-start;">
      ...
      <div style="display:flex;gap:8px;align-items:center;">
        ...
        <button onclick="exportRappelICS(${JSON.stringify(rappel).replace(/"/g, '&quot;')}, '${rucherLabel.replace(/'/g, "\\'")}');this.closest('#toast').classList.remove('show');"
  ...
  // Disparaît après 6s (plus long pour laisser le temps de cliquer)
...
async function saveJournal() {
  ...
  if (visitId) {
    try {
      if (isEditing) {
        // Supprimer les anciens médias avant de re-uploader
        await deleteAllVisitPhotos(visitId);
        await deleteVisitAudio(visitId);
      }
      ...
      if (currentVisitPhotos.length) {
        ...
        for (const p of currentVisitPhotos) {
          ...
          const url      = await uploadVisitPhotoToFirestore(visitId, blob, filename);
          visitPhotos[visitId].push(url);
        }
      } else if (isEditing) {
      ...
      if (currentVisitAudio?.blob) {
        const dataUrl = await uploadVisitAudioToFirestore(visitId, currentVisitAudio.blob, currentVisitAudio.duration);
        visitAudio[visitId] = { data: dataUrl, duration: currentVisitAudio.duration };
      } else if (isEditing) {
  ...
  if (entry.rappel) {
    showICSPrompt(entry.rappel, getRucherLabel(entry.rucher));
  } else {
