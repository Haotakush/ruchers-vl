...
/* ---- PHOTOS — Firebase Storage avec compression ---- */
async function addPhoto(rid, input) {
  const files = Array.from(input.files);
  if (!files.length) return;
  toast('📸 Compression et upload en cours…');
  if (!rucherPhotos[rid]) rucherPhotos[rid] = [];
  let processed = 0;
  for (const file of files) {
    try {
      const blob     = await compressImage(file, 2);
      const filename = `${Date.now()}_${Math.random().toString(36).substr(2,6)}.jpg`;
      const url      = await uploadPhotoToStorage(rid, blob, filename);
      rucherPhotos[rid].push(url);
      processed++;
    } catch (err) {
