...
/**
 * Supprimer un rucher après confirmation
 * @param {number} idx - Index dans le tableau RUCHERS
 */
function deleteRucher(idx) {
  const r = RUCHERS[idx];
...
/** Sauvegarder un nouveau rucher depuis le formulaire modal */
function saveNewRucher() {
...
/* ------------------------------------------------------------
   SELECTS — Mise à jour globale
   Appelée après tout changement dans RUCHERS
   ------------------------------------------------------------ */

/** Mettre à jour tous les selects de l'app avec la liste actuelle des ruchers */
...
/**
 * Ajouter une ou plusieurs photos à un rucher
 * Les photos sont stockées en base64 dans le localStorage
 * @param {string} rid    - ID du rucher
...
/** Générer le tableau du calendrier mellifère */
...
/** Générer le tableau de transhumance pour tous les ruchers */
...
/**
 * Cocher/décocher un mois dans la transhumance
 * @param {string}  key  - Clé ex. "R01_A_3"
 * @param {string}  type - "A" | "D" | "R"
