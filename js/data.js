...
/* ------------------------------------------------------------
   ...
   Initialisées avec les valeurs par défaut.
   Les vraies données sont chargées depuis Firestore après connexion
   (voir firebase-db.js → loadAllDataFromFirestore)
   ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   ...
   Écrivent dans Firestore ET dans localStorage (backup local)
   ------------------------------------------------------------ */

/** Sauvegarder la liste des ruchers */
function saveRuchers() {
  // Backup localStorage
  localStorage.setItem('ruchers', JSON.stringify(RUCHERS));
  // Firestore : chaque rucher est sauvé individuellement
  // (appelé directement via saveRucherToFirestore dans ruchers.js)
}
