...
async function loadVisitPhotos() {
  try {
    ...
    snap.forEach(doc => {
      const { visitId, data } = doc.data();
      if (!visitPhotos[visitId]) visitPhotos[visitId] = [];
      visitPhotos[visitId].push(data);
    });
  } catch (err) {
...
async function loadVisitAudio() {
  try {
    ...
    snap.forEach(doc => {
      const { visitId, data, duration } = doc.data();
      visitAudio[visitId] = { data, duration };
    });
  } catch (err) {
