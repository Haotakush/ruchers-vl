...
function startRealtimeListeners() {
  ...
  userRef().collection('journal').orderBy('date', 'desc')
    .onSnapshot(snapshot => {
      // On reconstruit journalData depuis Firestore (source unique de vérité)
      // On ne filtre plus hasPendingWrites — les écritures locales Firestore
      // sont cohérentes et servent de feedback immédiat (fonctionne offline aussi)
      journalData = [];
      snapshot.forEach(doc => journalData.push({ ...doc.data(), _docId: doc.id }));
      // Maintenir localStorage à jour (fallback offline)
