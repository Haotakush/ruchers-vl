/* ============================================================
   ...
   v1.1 — PDF dans nouvel onglet avec bouton retour
   ============================================================ */
...
function exportPDF() {
    ...
    <head>
      ...
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pdf-topbar { position: sticky; top: 0; background: #1A1208; color: #F5A623; padding: 14px 20px; display: flex; align-items: center; gap: 16px; z-index: 100; box-shadow: 0 2px 8px rgba(0,0,0,0.3); }
        .pdf-back-btn { background: rgba(245,166,35,0.15); border: 1px solid rgba(245,166,35,0.4); color: #F5A623; padding: 8px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: Arial, sans-serif; display: flex; align-items: center; gap: 6px; }
        .pdf-back-btn:hover { background: rgba(245,166,35,0.3); }
        .pdf-print-btn { background: #D4820A; border: none; color: white; padding: 8px 20px; border-radius: 8px; font-size: 13px; font-weight: 700; cursor: pointer; font-family: Arial, sans-serif; margin-left: auto; display: flex; align-items: center; gap: 6px; }
        .pdf-print-btn:hover { background: #F5A623; }
        .pdf-title { font-size: 14px; font-weight: 700; }
        .pdf-content { max-width: 900px; margin: 0 auto; padding: 24px 20px 60px; }
        ...
        h2 { font-size: 13px; color: #8B4513; border-bottom: 1px solid #E8DCC8; padding-bottom: 6px; margin: 20px 0 12px; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 16px; }
        th { background: #1A1208; color: #F5A623; padding: 7px 10px; text-align: left; font-size: 10px; letter-spacing: 0.04em; }
        ...
        .badge-zone-bas { background: #E8F5E9; color: #2E7D32; }
        .badge-zone-mi { background: #FFF8E1; color: #F57F17; }
        .badge-zone-hauts { background: #E3F2FD; color: #1565C0; }
        ...
        .empty { text-align: center; padding: 30px; color: #A8916A; font-size: 12px; }
    ...
    <body>
      ...
      <div class="pdf-content">
        <div class="header-block">
          ...
          <p><strong style="color:#F5A623;">${PARAMS.nom || '—'}</strong> · NAPI ${PARAMS.napi || '—'} · SIRET ${PARAMS.siret || '—'}</p>
          <p>${PARAMS.adresse || '—'}</p>
  ...
  if (!w) {
    ...
    toast('📄 Registre ouvert dans un nouvel onglet');
    return;
  }
